...
num_clusters = 5
nxt_day_predict = 7
db_dir_res = 'db/results/random_state'
...
if __name__ == "__main__":
    ...
    size = len(stock.df.index)
    rdm_states = np.random.choice(len(stock.df.index), size = size, replace = False)

    file_writting = dict()

    for k, rdm_state in enumerate(rdm_states, start = 1):
        file_writting = {'ticker' : ticker, 'random_state' : str(rdm_state), 'time' : [], 'preds_comp' : []}
        try:
